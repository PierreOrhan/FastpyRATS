...
class Visualize:
    ...
    def data(self, X, labels=None, title='', figsize=None, s=5,
        ...
        if X.shape[1] == 2:
            plt.scatter(X[:,0], X[:,1], s=s, c=labels, cmap=cmap, vmin=vmin, vmax=vmax)
    ...
    def global_embedding_for_gui(self, y, labels, cmap0, color_of_pts_on_tear=None, cmap1=None,
        ...
        if d == 2:
            ax = fig.add_subplot()
            pts = ax.scatter(y[:,0], y[:,1], s=s, c=labels, cmap=cmap0, vmin=vmin, vmax=vmax)
            ax.axis('image')
        elif d == 3:
            ax = fig.add_subplot(projection='3d')
            pts = ax.scatter(y[:,0], y[:,1], y[:,2], s=s, c=labels, cmap=cmap0, vmin=vmin, vmax=vmax)
            set_axes_equal(ax)
        ...
        if (color_of_pts_on_tear is not None):
            ...
            if cmap1 == 'colorcube' and (len(color_of_pts_on_tear.shape)==1):
                ...
                if d == 2:
                    pts_boundary = ax.scatter(y[pts_on_tear,0], y[pts_on_tear,1],
                               s=s, c=color_of_pts_on_tear[pts_on_tear], cmap=cmap1, vmin=None, vmax=None)
                elif d==3:
                    pts_boundary = ax.scatter(y[pts_on_tear,0], y[pts_on_tear,1], y[pts_on_tear,2],
                               s=s, c=color_of_pts_on_tear[pts_on_tear], cmap=cmap1, vmin=None, vmax=None)
                    set_axes_equal(ax)
    ...
    def global_embedding_images_v2(self, X, img_shape, y, labels, cmap0, color_of_pts_on_tear=None, cmap1=None,
                         title='', offset_ratio=0.3, zoom=1, nx=8, ny=8, v_ratio=0.8, w_ratio=0.005,
        ...
        if color_of_pts_on_tear is not None:
            ...
            ax.scatter(y[mask,0], y[mask,1],
                       s=s, c=color_of_pts_on_tear[mask], cmap=cmap1)
        #ax.set_title(title)
    ...
    def visualize_intermediate_views(
        X, c, Utilde, figsize=(5,5), s=1, subsample_inds=[],
        view_color='r', alpha=0.25
    ):
