...
class IntermedViews:
    ...
    def best(self, d_e, U, local_param, intermed_opts):
        ...
        for eta in range(2,intermed_opts['eta_min']+1):
            ...
            while cost_star < np.inf:
                ...
                if intermed_opts['tracker']:
                    self.tracker[eta]['c_updates'].append((k, dest_k))
                ...
                if False and len_S > intermed_opts['len_S_thresh']: # do parallel update (almost never true)
                    ...
                    # cost_and_dest = Parallel(n_jobs=n_proc)(delayed(cost_of_moving)(k, d_e, neigh_ind[k], U_[k], local_param,
                    #                                                                 c, n_C, Utilde, eta, eta_max, intermed_opts) for k in S)
                    # for i in range(len(S)):
                    #     k = S[i]
                    #     np_cost[k], np_dest[k] = cost_and_dest[i]
                ###########################################
                ###########################################
    ...
    def get_state(self):
        state = {
            ...
            'n_C': self.n_C,
            'Utilde': self.Utilde,
            'intermed_param': self.intermed_param.get_state(),
    ...
    def set_state(self, state):
        ...
        self.c = state['c']
        self.n_C = state['n_C']
        self.Utilde = state['Utilde']
        self.intermed_param.set_state(state['intermed_param'])
